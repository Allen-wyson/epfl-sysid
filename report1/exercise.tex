...
In order to get a good trade off between the resolution of the estimates (samples per period $M = 2^n -1$) and the impact of measurement errors (number of periods $p$) we decide to choose a signal with 4 periods $p=8$ in a 7 bit $n=8$ shift register.
...
The results can be seen in Figure ~\ref{fig:correlation_impulse_responses}.

\begin{figure}[h]
	\centering
	\includegraphics[height=6cm]{figures/ffts.png}
	\caption{Fourier transform of the input signal $U$, the output $Y$ and the transfer function $G$}\label{fig:ffts}
\end{figure}
...
As we use a periodic signal, the truncation error is suppressed and the identified model approaches the true one very well. 
Figure ~\ref{fig:resp_noise} shows the frequency response after applying a PRBS signal with noise.
Since the impact of noise is higher in high frequencies, the deviation is much bigger in the region of high frequencies. 

\begin{figure}[h]
	\centering
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[width=\textwidth]{figures/resp_noiseless.png}
		\subcaption{Frequency response without noise}
		\label{fig:resp_noiseless}
	\end{subfigure}
	\begin{subfigure}{0.49\textwidth}
		\includegraphics[width=\textwidth]{figures/resp_noise.png}
		\subcaption{Frequency response with noise}
		\label{fig:resp_noise}
	\end{subfigure}
	\caption{Comparision of the frequency response of identified model with the true one}\label{fig:f_resp}
\end{figure}
